          ...
          child: AnimatedSwitcher(
            ...
            child: MediaQueryObserver(
              ...
              child: DecoratedBox(
                ...
                // FLIPPER PATCH (see third_party/device_preview_plus/PATCHES.md):
                // this was a LayoutBuilder. A LayoutBuilder owns its own
                // BuildScope, and Element.buildScope propagates to the WHOLE
                // subtree — so with the app hosted here, every rebuild in the
                // app ran during performLayout. Mounting or re-activating any
                // OverlayPortal there (a Tooltip, or a route push re-parenting
                // stacked's GlobalKey'd Navigator) throws from
                // _RenderTheater._addDeferredChild, and that method leaves its
                // guard latched, so the assert then repeats every frame for the
                // rest of the session.
                //
                // `mediaQuery` below is MediaQueryData.fromView(window) —
                // installed by the MediaQueryObserver just above and rebuilt on
                // didChangeMetrics — so its size equals the constraints this
                // root-level box used to receive. Both former uses of
                // `constraints` now read from it.
                child: Builder(
                  builder: (context) {
                    final mediaQuery = MediaQuery.of(context);
                    final isSmall = mediaQuery.size.width < 700;
                        ...
                        if (isToolbarVisible && isSmall)
                          Positioned(
                            ...
                            child: DevicePreviewSmallLayout(
                              ...
                              maxMenuHeight: mediaQuery.size.height * 0.5,
