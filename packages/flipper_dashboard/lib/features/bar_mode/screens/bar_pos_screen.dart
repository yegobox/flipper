...
class BarPosScreen extends HookConsumerWidget {
          ...
          Expanded(
            ...
            child: Column(
              children: [
                ...
                Expanded(
                  child: _BarPosCatalogPane(
                    branchId: branchId,
                    onAdd: (v) =>
                        _addItem(ref, v, tab, table, branchId, cashier),
                  ),
                ),
              ],
            ),
  ...
  Future<void> _addItem(
    ...
    await ProxyService.getStrategy(Strategy.capella).addLineToBarTab(
      ...
      variantId: v.id,
      productName: v.name,
      defaultPrice: v.retailPrice ?? 0,
      stock: (v.stock?.currentStock ?? 999).toInt(),
...
class _BarPosCatalogPane extends ConsumerStatefulWidget {
  const _BarPosCatalogPane({
  ...
  @override
  ConsumerState<_BarPosCatalogPane> createState() =>
      _BarPosCatalogPaneState();
}

class _BarPosCatalogPaneState extends ConsumerState<_BarPosCatalogPane> {
              ...
              return LayoutBuilder(
                builder: (context, constraints) {
                  return GridView.builder(
                    ...
                    gridDelegate:
                        barPosCatalogGridDelegate(constraints.maxWidth),
