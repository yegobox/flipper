...
// **************************************************************************
//
// REQUIRED IMPORTS in parent file (supplier.model.dart):
...
class SupplierDittoAdapter extends DittoSyncAdapter<Supplier> {
  SupplierDittoAdapter._internal();

  static final SupplierDittoAdapter instance = SupplierDittoAdapter._internal();
  ...
  Future<String?> _resolveBranchId({bool waitForValue = false}) async {
    ...
    if (branchId == null && kDebugMode) {
      debugPrint("Ditto hydration for Supplier timed out waiting for branchId");
    }
    return branchId;
  }
  ...
  @override
  Future<String?> documentIdForModel(Supplier model) async => model.id;

  @override
  Future<Map<String, dynamic>> toDittoDocument(Supplier model) async {
    return {
      "_id": model.id,
      "id": model.id,
      "custNm": model.custNm,
  ...
  @override
  Future<Supplier?> fromDittoDocument(Map<String, dynamic> document) async {
  ...
  static Future<void> _seed(DittoSyncCoordinator coordinator) async {
    if (_seeded) {
      if (kDebugMode) {
        debugPrint('Ditto seeding skipped for Supplier (already seeded)');
      }
      return;
    }
    ...
    try {
      ...
      final models = await Repository().get<Supplier>(
        query: query,
        policy: OfflineFirstGetPolicy.alwaysHydrate,
      );
      var seededCount = 0;
      for (final model in models) {
        await coordinator.notifyLocalUpsert<Supplier>(model);
        seededCount++;
      }
      if (kDebugMode) {
      ...
      if (kDebugMode) {
        debugPrint('Ditto seeding failed for Supplier: $error\n$stack');
      }
    }
  ...
  static final int _$SupplierDittoAdapterRegistryToken =
      DittoSyncGeneratedRegistry.register(
          (coordinator) async {
            await coordinator
                .registerAdapter<Supplier>(SupplierDittoAdapter.instance);
          },
          modelType: Supplier,
  ...
  static int get registryToken => _$SupplierDittoAdapterRegistryToken;
}
