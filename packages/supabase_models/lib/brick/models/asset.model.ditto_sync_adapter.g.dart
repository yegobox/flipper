...
// **************************************************************************
//
// REQUIRED IMPORTS in parent file (assets.model.dart):
...
class AssetsDittoAdapter extends DittoSyncAdapter<Assets> {
  AssetsDittoAdapter._internal();

  static final AssetsDittoAdapter instance = AssetsDittoAdapter._internal();
  ...
  @override
  String get collectionName => "assets";
  ...
  Future<String?> _resolveBranchId({bool waitForValue = false}) async {
    ...
    if (branchId == null && kDebugMode) {
      debugPrint("Ditto hydration for Assets timed out waiting for branchId");
    }
    return branchId;
  }
  ...
  @override
  Future<String?> documentIdForModel(Assets model) async => model.id;

  @override
  Future<Map<String, dynamic>> toDittoDocument(Assets model) async {
    return {
      "_id": model.id,
      "id": model.id,
  ...
  @override
  Future<Assets?> fromDittoDocument(Map<String, dynamic> document) async {
  ...
  static Future<void> _seed(DittoSyncCoordinator coordinator) async {
    if (_seeded) {
      if (kDebugMode) {
        debugPrint('Ditto seeding skipped for Assets (already seeded)');
      }
      return;
    }
    ...
    try {
      ...
      final models = await Repository().get<Assets>(
        query: query,
        policy: OfflineFirstGetPolicy.alwaysHydrate,
      );
      var seededCount = 0;
      for (final model in models) {
        await coordinator.notifyLocalUpsert<Assets>(model);
        seededCount++;
      }
      if (kDebugMode) {
      ...
      if (kDebugMode) {
        debugPrint('Ditto seeding failed for Assets: $error\n$stack');
      }
    }
  ...
  static final int _$AssetsDittoAdapterRegistryToken =
      DittoSyncGeneratedRegistry.register(
          (coordinator) async {
            await coordinator
                .registerAdapter<Assets>(AssetsDittoAdapter.instance);
          },
          modelType: Assets,
  ...
  static int get registryToken => _$AssetsDittoAdapterRegistryToken;
}
