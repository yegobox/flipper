...
// **************************************************************************
//
// REQUIRED IMPORTS in parent file (category.model.dart):
...
class CategoryDittoAdapter extends DittoSyncAdapter<Category> {
  CategoryDittoAdapter._internal();

  static final CategoryDittoAdapter instance = CategoryDittoAdapter._internal();
  ...
  Future<String?> _resolveBranchId({bool waitForValue = false}) async {
    ...
    if (branchId == null && kDebugMode) {
      debugPrint("Ditto hydration for Category timed out waiting for branchId");
    }
    return branchId;
  }
  ...
  @override
  Future<String?> documentIdForModel(Category model) async => model.id;

  @override
  Future<Map<String, dynamic>> toDittoDocument(Category model) async {
    return {
      "_id": model.id,
      "id": model.id,
      "active": model.active,
  ...
  @override
  Future<Category?> fromDittoDocument(Map<String, dynamic> document) async {
  ...
  static Future<void> _seed(DittoSyncCoordinator coordinator) async {
    if (_seeded) {
      if (kDebugMode) {
        debugPrint('Ditto seeding skipped for Category (already seeded)');
      }
      return;
    }
    ...
    try {
      ...
      final models = await Repository().get<Category>(
        query: query,
        policy: OfflineFirstGetPolicy.alwaysHydrate,
      );
      var seededCount = 0;
      for (final model in models) {
        await coordinator.notifyLocalUpsert<Category>(model);
        seededCount++;
      }
      if (kDebugMode) {
      ...
      if (kDebugMode) {
        debugPrint('Ditto seeding failed for Category: $error\n$stack');
      }
    }
  ...
  static final int _$CategoryDittoAdapterRegistryToken =
      DittoSyncGeneratedRegistry.register(
          (coordinator) async {
            await coordinator
                .registerAdapter<Category>(CategoryDittoAdapter.instance);
          },
          modelType: Category,
  ...
  static int get registryToken => _$CategoryDittoAdapterRegistryToken;
}
