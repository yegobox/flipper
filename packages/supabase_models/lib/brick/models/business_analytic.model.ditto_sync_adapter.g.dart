...
part of 'business_analytic.model.dart';
...
// **************************************************************************
//
// REQUIRED IMPORTS in parent file (businessanalytic.model.dart):
...
class BusinessAnalyticDittoAdapter extends DittoSyncAdapter<BusinessAnalytic> {
  BusinessAnalyticDittoAdapter._internal();

  static final BusinessAnalyticDittoAdapter instance =
      BusinessAnalyticDittoAdapter._internal();
  ...
  Future<int?> _resolveBranchId({bool waitForValue = false}) async {
    ...
    if (branchId == null && kDebugMode) {
      debugPrint(
          "Ditto hydration for BusinessAnalytic timed out waiting for branchId");
    }
    return branchId;
  }
  ...
  Future<DittoSyncQuery?> _buildQuery({required bool waitForBranchId}) async {
    ...
    if (whereParts.isEmpty) {
      if (waitForBranchId) {
        if (kDebugMode) {
          debugPrint(
              "Ditto hydration for BusinessAnalytic skipped because branch context is unavailable");
        }
        return null;
      }
      if (kDebugMode) {
        debugPrint(
            "Ditto observation for BusinessAnalytic deferred until branch context is available");
      }
      return const DittoSyncQuery(
        query: "SELECT * FROM business_analytics WHERE 1 = 0",
      );
    }

    final whereClause = whereParts.join(" OR ");
    return DittoSyncQuery(
      query: "SELECT * FROM business_analytics WHERE $whereClause",
      arguments: arguments,
    );
  }
  ...
  @override
  Future<String?> documentIdForModel(BusinessAnalytic model) async => model.id;

  @override
  Future<Map<String, dynamic>> toDittoDocument(BusinessAnalytic model) async {
    return {
      "_id": model.id,
      "id": model.id,
      ...
      "stockRemainedAtTheTimeOfSale": model.stockRemainedAtTheTimeOfSale,
      "taxRate": model.taxRate,
      "trafficCount": model.trafficCount,
  ...
  @override
  Future<BusinessAnalytic?> fromDittoDocument(
    ...
    return BusinessAnalytic(
      id: id,
      date: DateTime.tryParse(document["date"]?.toString() ?? "") ??
          DateTime.now().toUtc(),
      itemName: document["itemName"],
      price: document["price"],
      profit: document["profit"],
      unitsSold: document["unitsSold"],
      stockRemainedAtTheTimeOfSale: document["stockRemainedAtTheTimeOfSale"],
      taxRate: document["taxRate"],
      trafficCount: document["trafficCount"],
  ...
  static Future<void> _seed(DittoSyncCoordinator coordinator) async {
    if (_seeded) {
      if (kDebugMode) {
        debugPrint(
            'Ditto seeding skipped for BusinessAnalytic (already seeded)');
      }
      return;
    }
    ...
    try {
      ...
      final models = await Repository().get<BusinessAnalytic>(
        query: query,
        policy: OfflineFirstGetPolicy.alwaysHydrate,
      );
      var seededCount = 0;
      for (final model in models) {
        await coordinator.notifyLocalUpsert<BusinessAnalytic>(model);
        seededCount++;
      }
      if (kDebugMode) {
        debugPrint('Ditto seeded ' +
            seededCount.toString() +
            ' BusinessAnalytic record' +
            (seededCount == 1 ? '' : 's'));
      }
    } catch (error, stack) {
      if (kDebugMode) {
        debugPrint('Ditto seeding failed for BusinessAnalytic: $error\n$stack');
      }
    }
  ...
  static final int _$BusinessAnalyticDittoAdapterRegistryToken =
      DittoSyncGeneratedRegistry.register(
          (coordinator) async {
            await coordinator.registerAdapter<BusinessAnalytic>(
                BusinessAnalyticDittoAdapter.instance);
          },
          modelType: BusinessAnalytic,
  ...
  static int get registryToken => _$BusinessAnalyticDittoAdapterRegistryToken;
}
