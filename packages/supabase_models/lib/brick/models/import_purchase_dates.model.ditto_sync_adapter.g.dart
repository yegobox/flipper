...
part of 'import_purchase_dates.model.dart';
...
// **************************************************************************
//
// REQUIRED IMPORTS in parent file (importpurchasedates.model.dart):
...
class ImportPurchaseDatesDittoAdapter
    extends DittoSyncAdapter<ImportPurchaseDates> {
  ImportPurchaseDatesDittoAdapter._internal();

  static final ImportPurchaseDatesDittoAdapter instance =
      ImportPurchaseDatesDittoAdapter._internal();
  ...
  @override
  String get collectionName => "import_purchase_dates";
  ...
  Future<String?> _resolveBranchId({bool waitForValue = false}) async {
    ...
    if (branchId == null && kDebugMode) {
      debugPrint(
          "Ditto hydration for ImportPurchaseDates timed out waiting for branchId");
    }
    return branchId;
  }
  ...
  @override
  Future<String?> documentIdForModel(ImportPurchaseDates model) async =>
      model.id;

  @override
  Future<Map<String, dynamic>> toDittoDocument(
      ImportPurchaseDates model) async {
    return {
      "_id": model.id,
      "id": model.id,
  ...
  @override
  Future<ImportPurchaseDates?> fromDittoDocument(
    ...
    return ImportPurchaseDates(
      id: id,
      lastRequestDate: document["lastRequestDate"],
  ...
  static Future<void> _seed(DittoSyncCoordinator coordinator) async {
    if (_seeded) {
      if (kDebugMode) {
        debugPrint(
            'Ditto seeding skipped for ImportPurchaseDates (already seeded)');
      }
      return;
    }
    ...
    try {
      ...
      final models = await Repository().get<ImportPurchaseDates>(
        query: query,
        policy: OfflineFirstGetPolicy.alwaysHydrate,
      );
      var seededCount = 0;
      for (final model in models) {
        await coordinator.notifyLocalUpsert<ImportPurchaseDates>(model);
        seededCount++;
      }
      if (kDebugMode) {
      ...
      if (kDebugMode) {
        debugPrint(
            'Ditto seeding failed for ImportPurchaseDates: $error\n$stack');
      }
    }
  ...
  static final int _$ImportPurchaseDatesDittoAdapterRegistryToken =
      DittoSyncGeneratedRegistry.register(
          (coordinator) async {
            await coordinator.registerAdapter<ImportPurchaseDates>(
                ImportPurchaseDatesDittoAdapter.instance);
          },
          modelType: ImportPurchaseDates,
  ...
  static int get registryToken =>
      _$ImportPurchaseDatesDittoAdapterRegistryToken;
}
