  ...
  Future<DittoSyncQuery?> _buildQuery({required bool waitForBranchId}) async {
    ...
    if (whereParts.isEmpty) {
      if (waitForBranchId) {
        if (kDebugMode) {
          debugPrint(
              "Ditto hydration for Device skipped because branch context is unavailable");
        }
        return null;
      }
      if (kDebugMode) {
        debugPrint(
            "Ditto observation for Device deferred until branch context is available");
      }
      return const DittoSyncQuery(
        query: "SELECT * FROM devices WHERE 1 = 0",
      );
    }
