...
// **************************************************************************
//
// REQUIRED IMPORTS in parent file (sar.model.dart):
...
class SarDittoAdapter extends DittoSyncAdapter<Sar> {
  SarDittoAdapter._internal();

  static final SarDittoAdapter instance = SarDittoAdapter._internal();
  ...
  @override
  String get collectionName => "sars";
  ...
  Future<String?> _resolveBranchId({bool waitForValue = false}) async {
    ...
    if (branchId == null && kDebugMode) {
      debugPrint("Ditto hydration for Sar timed out waiting for branchId");
    }
    return branchId;
  }
  ...
  @override
  Future<String?> documentIdForModel(Sar model) async => model.id;

  @override
  Future<Map<String, dynamic>> toDittoDocument(Sar model) async {
    return {
      "_id": model.id,
      "id": model.id,
      "sarNo": model.sarNo,
  ...
  @override
  Future<Sar?> fromDittoDocument(Map<String, dynamic> document) async {
  ...
  static Future<void> _seed(DittoSyncCoordinator coordinator) async {
    if (_seeded) {
      if (kDebugMode) {
        debugPrint('Ditto seeding skipped for Sar (already seeded)');
      }
      return;
    }
    ...
    try {
      ...
      final models = await Repository().get<Sar>(
        query: query,
        policy: OfflineFirstGetPolicy.alwaysHydrate,
      );
      var seededCount = 0;
      for (final model in models) {
        await coordinator.notifyLocalUpsert<Sar>(model);
        seededCount++;
      }
      if (kDebugMode) {
      ...
      if (kDebugMode) {
        debugPrint('Ditto seeding failed for Sar: $error\n$stack');
      }
    }
  ...
  static final int _$SarDittoAdapterRegistryToken =
      DittoSyncGeneratedRegistry.register(
          (coordinator) async {
            await coordinator.registerAdapter<Sar>(SarDittoAdapter.instance);
          },
          modelType: Sar,
  ...
  static int get registryToken => _$SarDittoAdapterRegistryToken;
}
