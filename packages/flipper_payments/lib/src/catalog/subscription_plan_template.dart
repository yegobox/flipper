...
class SubscriptionPlanTemplate {
  ...
  /// Explicit daily price, when the tier sets one. Null derives it from
  /// [monthlyPrice] — see [dailyPriceFromMonthly].
  ...
  factory SubscriptionPlanTemplate.fromSupabaseJson(
    ...
    return SubscriptionPlanTemplate(
      ...
      dailyPrice: (row['daily_price'] as num?)?.toInt(),
  ...
  /// Recurring amount for one billing period at [cadence].
  ///
  /// Mirrors `Template::price_for_cadence` on the server, which is the figure
  /// actually charged. Add-ons are priced monthly, so a daily plan divides the
  /// whole subtotal — pricing the tier daily but the add-ons monthly would
  /// charge a month of add-on for a day of service.
  double calculateTotalFor({
    ...
    switch (cadence) {
      ...
      case BillingCadence.daily:
        final explicit = dailyPrice;
        if (explicit != null && explicit > 0) {
          // An explicit daily price covers the tier only; add-ons still
          // divide, otherwise a 30,000/month add-on would be free on a daily
          // plan.
          final addonsMonthly = monthly - monthlyPrice;
          return (explicit + dailyPriceFromMonthly(addonsMonthly)).toDouble();
        }
        return dailyPriceFromMonthly(monthly).toDouble();
    }
  }
