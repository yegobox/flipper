...
class DodoClient {
  ...
  Future<http.Response> _send(
    ...
    try {
      ...
      talker.error('Dodo $what: no response from $url within ${_timeout.inSeconds}s');
      throw DodoException(
      ...
      talker.error('Dodo $what: could not reach $url — $e');
      throw DodoException(
  ...
  Future<DodoHealth> health() async {
    try {
      ...
      talker.info('Dodo health: $health');
      return health;
    } catch (e) {
      talker.warning('Dodo health check failed ($e); card payment stays hidden');
      return DodoHealth.unavailable;
    }
  }
  ...
  Future<DodoStartResult> startSubscription({
    ...
    if (result.planId.isEmpty || result.dodoSubscriptionId.isEmpty) {
      ...
      talker.error('Dodo start: HTTP ${response.statusCode} with no ids. ${response.body}');
      throw DodoException(
  ...
  Future<DodoSubscriptionStatus> syncSubscription(String planId) async {
    ...
    talker.info('Dodo sync $id → $status');
    return status;
  }
