...
class DodoCardCheckout {
  ...
  Future<DodoSubscriptionStatus?> awaitEntitlement(
    ...
    while (DateTime.now().isBefore(deadline)) {
      ...
      if (status != null) {
        ...
        if (status.entitled) {
          talker.info('Dodo: plan $planId is entitled after $attempt polls');
          return status;
        }
    ...
    talker.warning('Dodo: gave up waiting on plan $planId after $timeout');
    return last;
  }
  ...
  Future<bool> _open(String link) async {
    ...
    if (uri == null) {
      talker.error('Dodo: unusable checkout link "$link"');
      return false;
    }
    try {
      final opened = await _openLink(uri);
      if (!opened) talker.warning('Dodo: the platform refused to open $uri');
      return opened;
    } catch (e) {
      talker.error('Dodo: could not open the checkout page: $e');
      return false;
    }
  }
  ...
  Future<DodoSubscriptionStatus?> _safeStatus(
    ...
    try {
      ...
      if (forceSync) {
        talker.warning('Dodo sync failed ($e); falling back to a plain read');
        return _safeStatus(planId);
      }
      talker.warning('Dodo status read failed: $e');
      return null;
    } catch (e) {
      talker.warning('Dodo status read failed: $e');
      return null;
