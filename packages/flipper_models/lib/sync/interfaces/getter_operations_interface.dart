...
abstract class GetterOperationsInterface {
  Future<Device?> getDevice({required String phone, required String linkingCode});
  ...
  FutureOr<List<TransactionPaymentRecord>> getPaymentType({required String transactionId});
  ...
  Future<List<Product>> getProducts({String? key, int? prodIndex, required int branchId});
  Future<Receipt?> getReceipt({required String transactionId});
  FutureOr<Tenant?> getTenant({int? userId, int? pin});
  Future<({double expense, double income})> getTransactionsAmountsSum({required String period});
  Future<Plan?> getPaymentPlan({required int businessId});
  Future<bool> hasActiveSubscription({required int businessId, required HttpClientInterface flipperHttpClient});
