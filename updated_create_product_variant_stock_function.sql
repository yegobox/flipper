...
BEGIN
    ...
    FOR var_record IN SELECT
        ...
        NULLIF(x.tt_cat_cd, '') AS tt_cat_cd,
        x.property_ty_cd,
        x.room_type_cd,
        ...
        CASE WHEN x.tot_wt = '' OR x.tot_wt IS NULL OR x.tot_wt = '0' THEN NULL ELSE x.tot_wt::INTEGER END AS tot_wt,
        CASE WHEN x.net_wt = '' OR x.net_wt IS NULL OR x.net_wt = '0' THEN NULL ELSE x.net_wt::INTEGER END AS net_wt,
        CASE WHEN x.isrcc_cd = '' OR x.isrcc_cd IS NULL THEN NULL ELSE x.isrcc_cd END AS isrcc_cd,
        CASE WHEN x.isrc_amt = '' OR x.isrc_amt IS NULL OR x.isrc_amt = '0' THEN NULL ELSE x.isrc_amt::INTEGER END AS isrc_amt,
        CASE WHEN x.dc_amt = '' OR x.dc_amt IS NULL OR x.dc_amt = '0' THEN NULL ELSE x.dc_amt::DECIMAL END AS dc_amt,
        CASE WHEN x.taxbl_amt = '' OR x.taxbl_amt IS NULL OR x.taxbl_amt = '0' THEN NULL ELSE x.taxbl_amt::DECIMAL END AS taxbl_amt,
        CASE WHEN x.tax_amt = '' OR x.tax_amt IS NULL OR x.tax_amt = '0' THEN NULL ELSE x.tax_amt::DECIMAL END AS tax_amt,
        CASE WHEN x.tot_amt = '' OR x.tot_amt IS NULL OR x.tot_amt = '0' THEN NULL ELSE x.tot_amt::DECIMAL END AS tot_amt,
        x.expiration_date,
        x.unit,
        x.product_name,
        x.category_id,
